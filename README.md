...
춘천·양양·서귀포·평창·제주시·태안·홍천·가평의 후보 508건을 지역별로 준비했습니다.
`npm run data:regions`로 지역코드와 건수를 확인할 수 있습니다. 현재 화면과 DB에는 연결하지 않았습니다.
나중에 연결하려면 **“REGION_DATA_GUIDE.txt를 읽고 준비된 춘천 데이터를 웹에 연결해줘”**처럼 요청하세요.
서버 로더와 지역별 요청 예시는 [지역 데이터 사용 안내](docs/REGION_DATA_GUIDE.txt)에 있습니다.
