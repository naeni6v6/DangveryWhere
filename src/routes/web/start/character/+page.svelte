<script lang="ts">
   ...
   * 기다릴 것도, 부를 서버도 없습니다.
   *
   * 튜토리얼(/web/start) 마지막 단계와 [우리 강아지](/web/dog) 에서 들어옵니다.
  ...
  /**
   * 조절값. 밑그림이 바뀌면(= 견종이 정해지면) 그 그림의 본래 털 색에서 다시 시작합니다.
   * 강아지 정보를 늦게 읽어 오는 경우가 있어, 처음 한 번은 값이 채워진 뒤에 맞춰 줘요.
   */
  let params = $state<CharacterParams>({ ...DEFAULT_PARAMS });
  let paramsFor = '';
  $effect(() => {
    if (paramsFor === base.key) return;
    paramsFor = base.key;
          ...
          <div class="customize">
            <div class="custom-stage">
              ...
              <div class="trait-card">
                <span class="trait-label"><PawPrint size={14} />밑그림으로 쓴 캐릭터</span>
                <ul class="chips">
