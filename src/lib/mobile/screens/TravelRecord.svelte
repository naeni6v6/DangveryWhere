<script lang="ts">
  ...
  function readCoarse(): string[] {
    try {
      ...
      return Array.isArray(saved)
        ? saved.filter((v): v is string => typeof v === 'string' && validIds.has(v))
        : [];
    } catch {
      return [];
    }
  ...
  async function select(district: GangwonDistrict) {
    selectedCode = selectedCode === district.code ? null : district.code;
    if (selectedCode) {
      await tick();
      districtCard?.scrollIntoView({
  ...
  const mapOrder = $derived(
    [...districts].sort((a, b) => Number(a.code === selectedCode) - Number(b.code === selectedCode))
  );
</script>
  ...
  <div class="page">
    ...
    <div class="stamp-layout">
      ...
      <aside class="side">
        ...
        <section bind:this={districtCard} class="district-card" aria-live="polite">
          {#if selected}
            ...
            {#if region}
              <a class="district-explore" href={`${basePath}/explore?region=${region.id}`}>
