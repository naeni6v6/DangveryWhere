<script lang="ts">
  ...
  import { isTutorialDone, markTutorialDone } from '$lib/web/onboarding';
  ...
  function back() {
    if (step === 0) goto(basePath || '/mobile');
    else go(step - 1);
  }
  function skip() {
    markTutorialDone();
  ...
  async function finishToCharacter() {
    await finishTo(`${basePath}/start/character`, true);
  }
  async function finishTo(path: string, withDog = false) {
    if (saving) return;
    if (!profile) return go(3);
    saving = true;
    const previousIds = new Set(store.dogs.map((dog) => dog.id));
    await store.applyDog(profile);
    if (!store.dogs.some((dog) => !previousIds.has(dog.id))) {
      saving = false;
      return;
    }
    markTutorialDone();
  ...
  <form class="card" onsubmit={next} novalidate>
        ...
        {:else}
              ...
              {#key previewBreed?.key ?? (breedValue ? 'mystery' : '')}
                {#if previewBreed}
                  <img
                    ...
                    alt={previewBreed.label}
                    draggable="false"
                  />
                {:else if breedValue}
                  <!-- 기타·믹스·아직 캐릭터가 없는 견종: 미지의 실루엣 -->
                  <img
                    class="breed-model mystery"
                    src={MYSTERY_IMAGE}
                    alt="미지의 강아지"
                    draggable="false"
                  />
              ...
              {#each breeds as breed (breed.key)}
                ...
                >
                  <span class="option-thumb"
                    ><img src={breedThumb(breed)} alt="" loading="lazy" /></span
                  >
          ...
          {:else if step === 3}
            <div class="size-stage">
              <div
                class="size-figure"
                class:has-model={Boolean(previewBreed)}
    ...
    <footer class="card-foot">
      ...
      <button
        type="submit"
        class="btn-next"
        class:finish={step === steps.length - 1}
