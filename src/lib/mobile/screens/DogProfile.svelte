<script lang="ts">
  ...
  let { data, basePath = '' }: { data: AppData; basePath?: string } = $props();
        ...
        <div class="dog-grid">
          {#each store.dogs as dog (dog.id)}
            ...
            <article class="dog-card" class:current>
              ...
              <div class="card-copy">
                <strong
                  >{dog.name}{#if current}<span class="card-flag">선택됨</span>{/if}</strong
                >
        ...
        <section class="dog-stage">
          <div class="stage-top">
            <span class="stage-badge"
              ><Sparkles size={14} />{character ? '내 캐릭터' : '3D 미리보기'}</span
            >
          ...
          <div class="stage-caption">
            ...
            {:else}
              ...
              <span>아래에 정보를 입력하면 여기에 나타나요.</span>
            {/if}
          </div>

          <p class="stage-note">견종을 고르면 캐릭터가 바뀌고, 체급을 바꾸면 크기가 달라져요.</p>
          {#if editing}
            <a class="stage-character" href={`${basePath}/start/character?dog=${editing.id}`}>
              ...
              {#if editing}
                ...
                >
                  ...
                  <span class="go-copy">
                    ...
                    <span
                      >{editing.weight}kg · {sizeNames[editing.size]} 조건에 맞는 곳만 골라서 보여드려요</span
                    >
                  </span>
