<script lang="ts">
  ...
  type Provider = { id: 'kakao' | 'naver' | 'google'; label: string; name: string; href: string | null };
...
>
  ...
  <img class="login-logo" src="/logo.png" alt="" width="56" height="56" />
  <h2 id="web-login-title">댕브리웨어</h2>
  ...
  <div class="login-buttons">
    {#each providers as provider (provider.id)}
      ...
      {:else}
        <button type="button" class={`login-button ${provider.id}`} onclick={() => pending(provider)}>
          {@render logo(provider.id)}<span>{provider.label}</span>
        </button>
      {/if}
    {/each}
  ...
  <p class="login-foot">
    ...
    <em>* 카카오·네이버·구글 로그인이 연동될 예정이에요. 지금은 로그인 없이도 모든 장소를 볼 수 있어요.</em>
  </p>
</dialog>
  ...
  <span class="login-icon" aria-hidden="true">
    ...
    {:else if id === 'naver'}
      <svg viewBox="0 0 24 24" width="16" height="16"
        ><path fill="currentColor" d="M15.6 12.8 8.1 2H2v20h6.4V11.2L15.9 22H22V2h-6.4v10.8Z" /></svg
      >
    {:else}
